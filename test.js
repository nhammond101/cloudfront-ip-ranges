...
main
  ...
  .then(versionedIPs => {
    if (versionedIPs.V4.length > 0 && versionedIPs.V6.length > 0) {
      console.log(
        `Success for versioned: Got ${versionedIPs.V4.length} V4 and ${versionedIPs.V6.length} V6 IPs`,
      );
    } else {
  ...
  .then(() => {
    ...
    return main.updateTrustProxy(mockExpressApp).then(() => {
      if (mockExpressApp['trust proxy'].length > 0) {
        console.log(
          `Success for direct trust proxy update: ${mockExpressApp['trust proxy'].length} proxies were set`,
        );
        return;
      } else {
